...
function apply()
{
	...
	while true; do
		...
		if [ "${pd_ready_num}" -eq 1 ] && [ "${tikv_ready_num}" -eq 1 ] && [ "${tidb_ready_num}" -eq 1 ]; then
			break
		fi
